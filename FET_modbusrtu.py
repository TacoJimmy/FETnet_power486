...
def read_3p3w_meter(PORT,ID,loop):
    ...
    try:
        ...
        MainPW_meter[0] =  round(pw_va[0] * 0.1)
        MainPW_meter[1] =  round(pw_cur[0] * 0.01)
        MainPW_meter[2] =  round(pw_cur[1] * 0.01)
        MainPW_meter[3] =  round(pw_cur[2] * 0.01)
        MainPW_meter[4] =  round(pw_power[0] * 0.01) 
        MainPW_meter[5] =  round(pw_pf[0]*0.001)
        MainPW_meter[6] =  round((pw_consum[1] + pw_consum[0] * 65535)*0.1)
        MainPW_meter[7] = 1 
        master.close()
        time.sleep(0.5)
        return (MainPW_meter)
